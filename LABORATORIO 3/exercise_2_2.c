...
void exercise_2_2(void){
	...
	while(1) {
		...
		if((GPIOC->IDR & (1U << 13)) == 0){
			GPIOA->ODR &= ~(1U<<6);				// apaga el led del PA6
			GPIOA->ODR |= (1U<<7);				// enciende el led del PA7
		}else{
			GPIOA->ODR &= ~(1U<<7);     // apaga el led del PA7	
		}
		
		if((GPIOC->IDR & (1U<<14)) == 0){  
			GPIOA->ODR &= ~(1U<<6);
			GPIOA->ODR |= (1U<<8);     // enciende el led del PA8
		}else{
			GPIOA->ODR &= ~(1U<<8);     // apaga el led del PA8
